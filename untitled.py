...
def findNear(node,point1,point2,lenArr):
  ...
  boolZero = lenArr[0] != 0 and lenArr[1] != 0  and lenArr[2] != 0  and lenArr[3] != 0 
  for i in range(0,4,1):
    if i != point1 and i != point2:
      print(i)
      disA = twodis(node[0],beaconPoint[str(i)])
      disB = twodis(node[1],beaconPoint[str(i)])

      diffA = abs(disA-lenArr[i])
      diffB = abs(disB-lenArr[i])

      print(i,lenArr[i])
      print('diffA',diffA,'diffB',diffB)
      if(diffA>diffB):
        pointNow = node[0]
      elif(diffA<diffB):
        pointNow = node[1]
      # twodis(node[0],beaconPoint[str(i)])
      if(boolZero and checkIn):
        print('boolZero')
        # inMove()
      print(pointNow)
      return
